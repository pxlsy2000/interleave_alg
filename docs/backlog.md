...
## BL-004：提供 Terminal CLI 与纯网页两套 UI

**状态：待办**

**期望结果：** 同一套 Interleave 分析能力提供两个完整入口：一套是纯 Terminal Command Line 工具，适合脚本、自动化和批量运行；另一套是纯网页 UI，用户不需要打开 terminal，即可在浏览器中完成输入、校验、运行和查看 HTML 结果。

- [ ] 保持 CLI 为独立、完整的使用方式，不要求安装或启动网页 UI。
- [ ] 让网页 UI 覆盖完整闭环：创建或导入输入、编辑配置、校验、选择 workload/case、运行分析并查看结果。
- [ ] 为 Mapping 和 Scenario 提供适合其结构的表单或编辑器，并在字段附近展示校验错误与 warning。
- [ ] 支持导入现有 YAML，且能把网页中创建或修改的输入导出为 CLI 可直接使用的文件。
- [ ] 让网页 UI 与 CLI 共用同一套 schema、校验、分析指标和报告数据，不能维护两份会逐渐分叉的业务逻辑。
- [ ] 保证相同输入在两套 UI 中产生相同的 Mapping 分类、workload 指标、warning、error 和确定性顺序。
- [ ] 复用 BL-001 的 HTML 可视化能力，在网页端直接浏览 workload 总览和详细结果。
- [ ] 支持保存、重新载入或分享一组可复现输入，同时明确其中是否包含本地路径或敏感 trace 数据。
- [ ] 明确网页运行时的资源限制、长任务反馈、取消行为和失败恢复方式。
- [ ] 确保网页 UI 支持键盘操作、清晰的文字标签、窄屏布局和基础可访问性。
- [ ] 增加 CLI 与网页端的契约一致性测试，以及从网页输入到结果展示的真实浏览器端到端测试。

**待确认：**

- 网页版采用浏览器内本地计算（例如 WASM）、本地服务后端，还是远程服务。
- 网页 UI 优先作为本地工具交付，还是同时提供可部署版本。
- 输入与结果默认只留在本机，还是支持可选的持久化与分享能力。

## 共享事项

- [ ] 将可视化建立在稳定的内部报告数据上，使普通 `run`、DDR 分析和网页 UI 共享样式、图标、可访问性和文件输出能力。
