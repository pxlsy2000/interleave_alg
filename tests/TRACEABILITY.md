...
ID. Test IDs use the form `<integration-test-target>::<test-function>`.
...
| 8.1-05 | Granule exceeds `2^52` after the intrinsic relation passes | `mapping_schema::enforces_cross_family_precedence_and_exact_reasons` |
| 8.1-06 | Target count or granule is not a power of two | `conformance_exclusions::non_power_of_two_targets_are_rejected_without_fallback`; `mapping_schema::enforces_cross_family_precedence_and_exact_reasons` |
| 8.1-07 | Target count exceeds 65,536 | `mapping_schema::rejects_target_cap_only_after_intrinsic_relation_passes` |
...
| 8.1-11 | `M` or explicit `L` has the wrong row count | `mapping_schema::validates_dimensions_taps_duplicates_and_conditional_l_rows` |
...
| 8.2-02 | Address is at least `2^A` | `conformance_mapping::a64_maximum_passes_and_exclusive_bound_fails_without_rows` |
...
| 8.2-06 | Base or stride is unaligned | `address_mapping::maps_documented_example_and_preserves_unaligned_offset` |
...
| 8.2-09 | Decoded scalar is 129 UTF-8 bytes | `yaml_contract::decoded_scalar_bound_rejects_plain_quoted_and_multibyte_key_values_at_plus_one` |
...
| 8.2-11 | Core integer first-match spellings use base-10 observations | `yaml_contract::core_integer_first_match_keeps_lexemes_and_separates_signed_base_prefixes` |
...
| 8.3-02 | Window is zero, duplicated, or list is empty | `scenario_schema::enforces_scalar_type_lexeme_and_range_ladders`; `scenario_schema::rejects_duplicate_names_windows_and_normalized_sweep_addresses` |
...
| 8.3-07 | Sweep base or stride value repeats | `conformance_scenario::duplicate_sweep_values_and_generated_carry_fail_before_analysis` |
| 8.3-08 | Multi-stream case has one stream | `conformance_scenario::unequal_and_single_stream_round_robin_keep_declared_order` |
| 8.3-09 | Multi-stream lengths differ | `conformance_scenario::unequal_and_single_stream_round_robin_keep_declared_order` |
...
| 8.3-15 | `Q=10,000,000`, `K=1,024` exceeds exact window work | `scenario_expand::exact_window_work_is_the_fifth_global_gate` |
...
| 8.4-02 | `W < N` | `metrics::counts_and_max_load_use_exact_unreduced_formulas` |
...
| 9.1-06 | Every Mapping preserves the byte offset | `address_mapping::natural_mapping_matches_direct_formula_for_every_small_address`; `address_mapping::explicit_mapping_matches_independent_xor_evaluation` |
...
| 9.2-02 | Unknown fields, duplicate fields, and wrong types are rejected | `scenario_schema::combined_failures_follow_declaration_and_source_order_without_fallback`; `yaml_contract::duplicate_key_uses_second_occurrence_and_terminal_safe_path`; `mapping_schema::applies_scalar_type_lexeme_and_name_gates` |
| 9.2-03 | Decoded-scalar exact/plus-one, ordering, precedence, and exits | `yaml_contract::decoded_scalar_bound_accepts_exact_plain_and_quoted_key_values`; `yaml_contract::decoded_scalar_bound_rejects_plain_quoted_and_multibyte_key_values_at_plus_one`; `run_cli::overlong_decoded_scenario_scalar_is_a_json_business_failure` |
| 9.2-04 | Core first-match and typed-v1 lexeme gates are independent | `yaml_contract::core_integer_first_match_keeps_lexemes_and_separates_signed_base_prefixes`; `scalar_contract::public_generic_integer_parser_is_distinct_from_address_parser` |
| 9.2-05 | Options, selection order, and exits match Chapters 5 and 7 | `cli_usage::duplicate_singleton_options_are_usage_failures_before_input_read`; `conformance_scenario::disabled_selection_and_repeated_case_names_are_deterministic`; `scalar_contract::typed_errors_cover_every_process_exit_class` |
...
| 9.2-07 | Failures leave no partial result or truncated output | `conformance_output::preflight_failure_leaves_existing_text_output_and_directory_untouched`; `map_cli::a_late_bad_address_suppresses_every_preceding_valid_row` |
...
| 9.3-04 | Aligned and staggered streams differ deterministically | `metrics::aligned_and_staggered_stream_metrics_match_hand_calculated_goldens` |
| 9.3-05 | Sweep order, IDs, and metrics are independent | `scenario_generate::sweep_combinations_remain_independent_and_base_major`; `run_cli::all_documented_kinds_expand_to_fourteen_ordered_reports` |
...
| 9.4-04 | JSON fields, ordering, addresses, and ratios match Chapter 6 | `json_golden::json_command_cases::map_success_preserves_order_and_canonicalizes_addresses`; `json_golden::json_command_cases::run_success_matches_ordered_multi_case_golden` |
